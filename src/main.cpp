...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float time_func = static_cast<float>(sin(glfwGetTime() * 2.0));

        float blue_level = time_func >= 0.2f ? time_func : 0.2f;

        // light properties
        lightingShader.setVec3("light.ambient", 0.f, 0.f, 0.1f);
        lightingShader.setVec3("light.diffuse", 0.f, 0.f, time_func);
        ...
        for (unsigned int i = 0; i < 10; i++) {
            ...
            float angle = 20.0f * (i+1)/(1.2f) * glfwGetTime();
            model =
